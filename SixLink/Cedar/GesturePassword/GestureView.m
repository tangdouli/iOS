    ...
    if (touch) {
        ...
        for (int i=0; i < _buttonArray.count; i++) {
            ...
            if (CGRectContainsPoint(buttonTemp.frame, touchPoint)) {
                ...
                [buttonTemp setNeedsDisplay];
                CGRect frameTemp = buttonTemp.frame;
                CGPoint point = CGPointMake(frameTemp.origin.x+frameTemp.size.width/2.0,frameTemp.origin.y+frameTemp.size.height/2.0);
...
-(void)touchesEnded:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event
{
    ...
    UBLog(@"resultString:%@",resultString);
...
- (void)drawRect:(CGRect)rect {
    ...
    // Drawing code
//        if (touchesArray.count<4)return;
        for (int i=0; i<touchesArray.count; i++) {
...
//            CGContextSetRGBStrokeColor(context, 2/255.f, 174/255.f, 240/255.f, 0.7);//线条颜色
            CGContextSetRGBStrokeColor(context, 0.145, 0.710,0.686,1);//线条颜色
//            [UIColor colorWithRed:0.145 green:0.710 blue:0.686 alpha:1]
